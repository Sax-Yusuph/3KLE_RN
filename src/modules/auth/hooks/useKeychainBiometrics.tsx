...
export default function useKeychainBiometrics({ username, password }: Credentials) {
	...
	useEffect(() => {
		...
		if ((Platform.OS === 'android' && supportedBiometry) || Platform.OS === 'ios') {
			Alert.alert(
				`Use ${supportedBiometry} next time`,
				`You will be prompt to use ${supportedBiometry}`,
				[
					{
						text: 'Yes',
						onPress: async () => {
...
export default function useKeychainBiometrics({
  username,
  password,
}: Credentials) {
  const [storeCredentials, setStoreCredentials] = useState<boolean>(false);
  ...
  useEffect(() => {
    ...
    if (
      (Platform.OS === 'android' && supportedBiometry) ||
      Platform.OS === 'ios'
    ) {
      Alert.alert(
        `Use ${supportedBiometry} next time`,
            ...
            onPress: async () => {
              try {
                await Keychain.setGenericPassword(username, password, {
                  ...
                  accessible:
                    Keychain.ACCESSIBLE.WHEN_PASSCODE_SET_THIS_DEVICE_ONLY, // iOS only
                  // service: '<ADD_APP_BUNDLE_ID>',
                });
                await AsyncStorage.setItem('hasCredentials', 'true');
              } catch (err) {
